...
ISO 8601 是国际标准化组织制定的日期和时间表示法标准。它定义了一种统一的、易于相互比较和排序的日期和时间表示方式。

**基本格式：**

- **日期**：`YYYY-MM-DD`（例如：`2025-05-13`）
- **时间**：`HH:MM:SS`（例如：`14:30:45`）
- **日期和时间**：`YYYY-MM-DDTHH:MM:SS`（例如：`2025-05-13T14:30:45`）
- **带时区**：`YYYY-MM-DDTHH:MM:SS±HH:MM`（例如：`2025-05-13T14:30:45+08:00`）或 `YYYY-MM-DDTHH:MM:SSZ`（UTC 表示，例如：`2025-05-13T06:30:45Z`）
...
| `MM` | 2 位月份（01-12）| `05` |
| `DD` | 2 位日期（01-31）| `13` |
| `T` | 日期和时间的分隔符 | `2025-05-13T14:30:45` |
| `HH` | 2 位小时（00-23）| `14` |
| `MM` | 2 位分钟（00-59）| `30` |
| `SS` | 2 位秒（00-59）| `45` |
| `±HH:MM` | 时区偏移量（相对于 UTC）| `+08:00`（东 8 区），`-05:00`（西 5 区） |
| `Z` | 表示 UTC 时区（相当于 ±00:00）| `2025-05-13T06:30:45Z` |
...
RFC 2822 和 RFC 7231 定义了在邮件头和 HTTP 协议中使用的日期格式标准。

**RFC 2822 格式（邮件头）：**

`Day, DD Mon YYYY HH:MM:SS ±HHMM`

示例：`Wed, 01 May 2025 12:00:00 +0800`

**符号说明：**

| 符号 | 含义 | 示例 |
| ---- | ---- | ---- |
| `Day` | 星期几的英文缩写（Mon、Tue、Wed、Thu、Fri、Sat、Sun）| `Wed` |
| `DD` | 2 位日期（01-31）| `01` |
| `Mon` | 月份的英文缩写（Jan、Feb、Mar、Apr、May、Jun、Jul、Aug、Sep、Oct、Nov、Dec）| `May` |
| `YYYY` | 4 位年份 | `2025` |
| `HH:MM:SS` | 时间（小时:分钟:秒）| `12:00:00` |
| `±HHMM` | 时区偏移量（相对于 UTC，±表示正或负偏移）| `+0800`（东 8 区），`-0500`（西 5 区） |

**RFC 7231 格式（HTTP 协议）：**

`Day, DD Mon YYYY HH:MM:SS GMT`

示例：`Wed, 01 May 2025 12:00:00 GMT`
...
| `Day` | 星期几的英文缩写（Mon、Tue、Wed、Thu、Fri、Sat、Sun）| `Wed` |
| `DD` | 2 位日期（01-31）| `01` |
| `Mon` | 月份的英文缩写（Jan-Dec）| `May` |
| `YYYY` | 4 位年份 | `2025` |
| `HH:MM:SS` | 时间（小时:分钟:秒）| `12:00:00` |
| `GMT` | 格林威治标准时间（固定为 UTC）| `GMT` |

**特点：**

- 包含星期几信息，便于人类阅读
- 使用英文月份缩写（Jan、Feb、Mar 等）
- 时区使用 GMT（格林威治标准时间）或 ±HHMM 偏移量表示
- 广泛应用于 HTTP 响应头（如 `Expires`、`Last-Modified`）和邮件协议

**常见应用场景：**

- HTTP Cookie 的 `Expires` 字段
- HTTP 响应头中的日期字段
- 电子邮件的发送时间
- API 返回的日期信息
...
UNIX 时间戳（Unix Timestamp）是指从 1970 年 1 月 1 日 00:00:00 UTC 开始经过的秒数（或毫秒数）。它是一种简洁的日期和时间表示方式，广泛应用于计算机系统和编程语言中。

**基本特点：**

- **秒级时间戳**：从 1970-01-01 00:00:00 UTC 起计算的秒数
  - 示例：`1746835200`（表示 2025-05-13 12:00:00 UTC）
- **毫秒级时间戳**：从 1970-01-01 00:00:00 UTC 起计算的毫秒数
  - 示例：`1746835200000`（表示 2025-05-13 12:00:00 UTC）
- **微秒级时间戳**：从 1970-01-01 00:00:00 UTC 起计算的微秒数
  - 示例：`1746835200000000`
...
- 表示方式简洁，只需一个整数
- 便于计算时间差和时间间隔
- 避免了时区转换的复杂性（统一使用 UTC）
- 易于在数据库和日志中存储和查询
- 不受地区语言和日期格式差异的影响

**常见应用场景：**

- 数据库中的时间戳字段（created_at、updated_at）
- 日志文件中的事件时间戳
- JavaScript 等编程语言中的时间表示（`Date.now()` 返回毫秒时间戳）
- API 响应中的时间信息
- 缓存过期时间的设置
- 性能监测和系统指标记录
