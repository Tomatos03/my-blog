...
CSS 选择器是用于定位和选择 HTML 元素的模式。通过选择器，我们可以对特定的元素应用样式。掌握各种选择器是编写高效 CSS 代码的基础。
...
1. **过度依赖 ID 选择器：** ID 优先级过高，不易覆盖
2. **频繁使用 !important：** 会导致样式冲突难以解决
3. **在属性选择器中使用通用匹配：** 性能较差
```css
/* ❌ 性能较差 */
[class*="btn"] {
    /* ... */
