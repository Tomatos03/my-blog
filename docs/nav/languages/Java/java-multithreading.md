...
1. 多个线程尝试访问`synchronized`保护的临界资源
2. Monitor对象中的owner记录当前线程，其他线程进入Entry List等待锁的释放(放入Entry List的线程处于Blocked状态)
3. 持有锁的线程手动调用`wait()`方法进入Wait Set等待被唤醒(线程此时处于Waiting状态)，或者正常执行完同步块释放锁(线程进入Terminated状态)
4. 调用`notify()`方法随机唤醒Wait Set中的一个线程，唤醒的线程从Wait Set移到Entry List，等待获取锁(线程此时处于Blocked状态)

![alt text](assets/image.png-1764591724252.png)

> [!NOTE]
> `notify()`方法会**随机**唤醒等待集合`WaitSet`中的一个线程.
> `notifyAll()`方法会唤醒等待集合`WaitSet`中的所有线程, 但被唤醒的线程仍然需要竞争锁, 只有获取到锁的线程才能继续执行同步块中的代码
